...
let dragEnteredModuleId

let modules = [
    { id: 1, name: 'First Module' },
    { id: 2, name: 'Last Module' },
    { id: 3, name: 'Last Module' },
]

let users = [
    { id: 1, fullName: 'John Doe', moduleId: 1 },
    { id: 2, fullName: 'Tolepov Asan', moduleId: 2 },
    { id: 3, fullName: 'Tom Karl', moduleId: 3 },
]
...
function renderModules(modulesArray = []) {
    ...
    modulesArray.map((item) => {
        ...
        id="module-${item.id}" class="w-[320px] min-w-[320px] max-w-[320px] border border-gray-300 rounded-md p-2 bg-[rgba(255, 255, 255, 0.2)] bg-opacity-30 backdrop-filter backdrop-blur">

        <!-- module header -->
        <header class="w-full bg-indigo-500 rounded-md p-2 flex justify-between items-center text-white">
            <h2>${item.name}</h2>
        ...
        <!-- divider -->
        <div class="w-full h-[1px] bg-gray-500 bg-opacity-30 my-2"></div>
    ...
    usersArray.map(item => {
        ...
        currentUserModuleEl.innerHTML += `
            <!-- user -->
                ...
                <h2 class="text-slate-700 hover:text-blue-500">${item.fullName}</h2>
...
function dragEnded() {
    document.querySelector(`#user-${dragStartedUserId}`).classList.remove('hidden')
